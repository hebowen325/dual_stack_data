...
/*type Options struct {
        DNSFile  string `short:"f" long:"Dnsfile" description:"A large file containing sorted, reversed domain names" required:"true"`
...
func DNSSearch(start int, end int, interval int) {
        ...
        var IPv6 string
        var IPv4 string
        f, err := os.Open(x + "/fdns_a/Record.txt")
        if err != nil {
                return
        }
        fi, err := f.Stat()
        ...
        for i := start; i <= end; i += interval {
                ...
                IPv6 = stringutil.Reverse(strings.Split(IPaddr[i], ",")[1])
                for t := i - 1; t >= 0; t-- {
                        if strings.Compare(strings.Split(IPaddr[i], ",")[0], strings.Split(IPaddr[t], ",")[0]) == 0 {
                                IPv6 = IPv6 + "," + stringutil.Reverse(strings.Split(IPaddr[t], ",")[1])
                        } else {
                                break
                        }
                }
                ...
                if err != nil {
                        ...
                        for _, result := range output {
                                if strings.Split(result, ",")[1] == domain_name {
                                        ...
                                        flag = true
                                        IPv4 = IPv4 + strings.Split(result, ",")[0]
                                }
                        }
                        if flag == true {
                                fmt.Printf("%s;%s;%s\n", domain_name, IPv4, IPv6)
                        }
                }
        }
        f.Close()
...
func main() {
        ...
        files, err := ioutil.ReadDir("/home/sgl/bwhe/Router/data/data_ipv6/")
        if err != nil {
                panic(err)
        }
        // 获取文件，并输出它们的名字
        for _, file := range files {
                if file.Name()[0:4] == "doma" {
                        data, err := ioutil.ReadFile("/home/sgl/bwhe/Router/data/data_ipv6/" + file.Name())
                        if err != nil {
